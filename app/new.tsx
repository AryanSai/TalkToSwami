...
const rotations = [2.5, -3.2, 1.8, -2.7, 4.1, -1.5, 3.6];
...
const CardStack = () => {
  ...
  const rotateTop = useRef(new Animated.Value(0)).current;
  const rotateNext = useRef(new Animated.Value(rotations[nextCard])).current;

  const rotateInterpolateTop = rotateTop.interpolate({
    inputRange: [-360, 360],
    outputRange: ['-360deg', '360deg'], // Rotate from 0 to 360 degrees
  });
  const rotateInterpolateNext = rotateNext.interpolate({
    inputRange: [-360, 360],
    outputRange: ['-360deg', '360deg'], // Rotate from 0 to 360 degrees
  });
  ...
  const handleCardSwap = () => {
    Animated.parallel([
      ...
      Animated.timing(rotateTop, {
        toValue: rotations[topCard],
        duration: 1000,
        useNativeDriver: true,
      }),
      Animated.timing(rotateNext, {
        toValue: 0,
        duration: 1000,
        useNativeDriver: true,
      }),
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View
        style={[
          styles.card,
          { transform: [{ translateY: translateYNext }, { rotate: `${rotations[nextCard]}deg` }], zIndex: 10 },
        ]}
      >
      ...
      <Animated.View
        style={[
          styles.card,
          { transform: [{ translateY: translateYTop }, { rotate: rotateInterpolateTop }], zIndex: 20 },
        ]}
      >
